              ...
              fml* destPtr)
{
    ...
    destPtr += blockIdx.z*inputRows*inputCols*INPUT_COMPONENTS + global_y*inputCols*INPUT_COMPONENTS + global_x*INPUT_COMPONENTS;
    srcPtr += blockIdx.z*(inputRows/POOL_ROWS)*(inputCols/POOL_COLS)*INPUT_COMPONENTS + (global_y/POOL_ROWS)*(inputCols/POOL_COLS)*INPUT_COMPONENTS + (global_x/POOL_COLS)*INPUT_COMPONENTS;
    ...
    for (u32 inputComponentIndex = 0; inputComponentIndex < INPUT_COMPONENTS; inputComponentIndex++)
    {
        ...
        if (isOutputThread)
        {
            fml src = srcPtr[inputComponentIndex];

            u32 therow = 0, thecol = 0;

            for (u32 r = 0; r < POOL_ROWS; r++)
            {
                for (u32 c = 0; c < POOL_COLS; c++)
                {
                    fml valhere = input_shared[(threadIdx.y+r) * BLOCK_SIZE_X + (threadIdx.x+c)];
                    if (valhere > value)
                    {
                        value = valhere;
                        therow = r;
            ...
            destPtr[therow*inputCols*INPUT_COMPONENTS + thecol*INPUT_COMPONENTS + inputComponentIndex] = src;
        }
