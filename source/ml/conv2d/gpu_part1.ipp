              ...
              fml* outputPtr, u32 outputRows, u32 outputCols)
{
    ...
    inputPtr += blockIdx.z * inputsPerBlock * inputRows * input_row_width;
    ...
    const fml* input_start;
    {
        ...
        outputPtr += blockIdx.z * inputsPerBlock * outputRows * outputCols * NUM_KERNELS + global_y/KERNEL_STEP_Y * outputCols * NUM_KERNELS + global_x/KERNEL_STEP_X * NUM_KERNELS;
    ...
    for (u32 inputIndex = 0, inputIndex2 = blockIdx.z * inputsPerBlock; inputIndex < inputsPerBlock && inputIndex2 < inputCount; inputIndex++, inputIndex2++)
    {
        // Copy all the input of this block into shared memory.
        {
            ...
            inputPtr += inputRows * input_row_width;
        }
        ...
        if (isOutputThread)
        {
            ...
            outputPtr += outputRows * outputCols * NUM_KERNELS;
        }

        // We can't loop back up and modify shared memory again until all threads are here.
        __syncthreads();
    }
}
              ...
              fml* outputPtr, u32 outputRows, u32 outputCols)
{
    ...
    const fml* input_start;
    {
        ...
        isInsideInput =
                    ((global_y >= 0) & (global_y < inputRows) &
                     (global_x >= 0) & (global_x < inputCols));
        inputPtr += blockIdx.z * inputsPerBlock * inputRows * inputCols * INPUT_COMPONENTS + global_y * inputCols * INPUT_COMPONENTS + global_x * INPUT_COMPONENTS;
    ...
    for (u32 inputIndex = 0, inputIndex2 = blockIdx.z * inputsPerBlock; inputIndex < inputsPerBlock && inputIndex2 < inputCount; inputIndex++, inputIndex2++)
    {
        ...
        inputPtr += inputRows * inputCols * INPUT_COMPONENTS;
        outputPtr += outputRows * outputCols * NUM_KERNELS;
    }
}
